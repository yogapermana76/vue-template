/**
 * Mock data for the Loket module — used when VITE_LOKET_MOCK=true.
...
// Programs
// ============================================
export const mockPrograms: LoketProgram[] = [
  {
    ID: 1,
    ProgramID: 101,
...
// Banners per program
// ============================================
export const mockBanners: Record<number, LoketBannerURL[]> = {
  1: [
...
// ============================================
export const mockCategories: Record<number, LoketCategory[]> = {
  1: [
...
// ============================================
export const mockSchedules: Record<number, LoketSchedule[]> = {
  1: [
...
// ============================================
export const mockStatistics: Record<number, LoketStatistics> = {
  1: {
...
export const mockMonitoringHeaders: LoketColumnHeader[] = [
...
const seedPrograms = new Map<number, LoketTicketItem[]>()

function generateProgramTickets(programId: number, count: number): LoketTicketItem[] {
  const categories = CATEGORY_NAMES_BY_PROGRAM[programId] ?? ['Regular']
  const items: LoketTicketItem[] = []
...
export function getMockMonitoringData(programId: number): LoketTicketItem[] {
...
// ============================================
export function getMockTicketDetail(programId: number, code: string): LoketTicketDetail | null {
  const items = getMockMonitoringData(programId)
  const item = items.find(x => x.TicketCode === code)
  if (!item) return null
  // Pass-through the enriched item — matching LoketTicketDetail shape.
  return { ...item, TicketCode: String(item.TicketCode) } as LoketTicketDetail
}
