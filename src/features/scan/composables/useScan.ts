...
import { useClaimTicket } from '@/composables/services'
...
export function useScan(programIdRef: () => number | undefined) {
  ...
  const verifyCode = async (code: string) => {
    ...
    try {
      const scanResponse = await ticketService.scanTicket({ code })
      const scanResult: ScanResult | undefined = scanResponse.data
