...
import {
  useLoketMonitoring,
  useLoketMonitoringExport,
  useLoketTicketDetail,
} from '@/composables/services/loket'
import type { LoketPagination } from '@/types/services/loket'
import {
  emptyMonitoringFilters,
  toMonitoringParams,
  type LoketMonitoringFilters,
...
export function useLoketMonitoringView(programIdRef: () => number | undefined) {
  const page = ref(1)
  const perPage = ref(DEFAULT_PAGE_SIZE)
  const filters = ref<LoketMonitoringFilters>(emptyMonitoringFilters())
  ...
  const listQuery = useLoketMonitoring({
    query: {
      page,
      size: perPage,
  ...
  const pagination = computed<LoketPagination | undefined>(() => listQuery.data.value?.pagination)
  ...
  const applyFilters = (next: LoketMonitoringFilters) => {
    filters.value = next
    page.value = 1
  ...
  const detailQuery = useLoketTicketDetail({
  ...
  const exportMutation = useLoketMonitoringExport()
