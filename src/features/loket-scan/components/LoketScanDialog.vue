<script setup lang="ts">
  ...
  import { LOKET_PROGRAMS_PAGE_SIZE } from '@/features/loket-dashboard'
  import { useLoketScan } from '../composables/useLoketScan'
  import { useGunScannerListener } from '../composables/useGunScannerListener'
  import { useLoketScanModal } from '../composables/useLoketScanModal'
  import { useScanCameraLifecycle } from '../composables/useScanCameraLifecycle'
  import LoketScanCamera from './LoketScanCamera.vue'
  import LoketManualCodeInput from './LoketManualCodeInput.vue'
  import LoketTicketPreviewCard from './LoketTicketPreviewCard.vue'
  import LoketMobileScannerView from './LoketMobileScannerView.vue'
  ...
  const programsQuery = useLoketPrograms({
    query: { page: 1, size: LOKET_PROGRAMS_PAGE_SIZE },
  ...
  const scan = useLoketScan(() => props.programId)
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      <template v-if="scannerMode === 'camera' && isScanningPhase">
        <LoketScanCamera
          ...
          @manual="setMode('gun')"
        />
        <LoketManualCodeInput @submit="scan.submitManual" />
      </template>
