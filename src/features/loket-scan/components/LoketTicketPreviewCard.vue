<script setup lang="ts">
  ...
  import LoketScanOrderSummary from './LoketScanOrderSummary.vue'
  import LoketScanTicketList from './LoketScanTicketList.vue'

  const props = defineProps<{
    scanResult: LoketScanResult
...
<template>
  <Card :class="['overflow-hidden border bg-linear-to-b shadow-xs', heroAccent]">
    ...
    <CardContent class="flex flex-col gap-5 py-5">
      <LoketScanOrderSummary v-if="order" :order="order" />
